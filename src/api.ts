...
import viveroRoutes from './routes/viveroRoutes';
...
coleccionesAdmin.forEach(coleccion => {
  ...
  app.put(`/api/${coleccion}/:id`, async (req, res) => {
    try {
      ...
      const documentoActualizado = await db.collection(coleccion).findOne({ _id: id as any });
...
// Rutas para viveros con sistema completo de clones
app.use('/api/viveros', viveroRoutes);
